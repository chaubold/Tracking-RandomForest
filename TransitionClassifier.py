...
def compute_features(raw_image, labeled_image, n1, n2):
    ...
    for i in range(0, n2 - n1):
        ...
        if len(raw_image.shape) < 5:
            tempnew1 = vigra.analysis.extractConvexHullFeatures(labeled_image[i][..., 0].squeeze().astype(np.uint32),
                                                                ignoreLabel=0)
            tempnew2 = vigra.analysis.extractSkeletonFeatures(labeled_image[i][..., 0].squeeze().astype(np.uint32))
            allFeat.append(dict(features.items() + tempnew1.items() + tempnew2.items()))
        else:
            allFeat.append(dict(features.items()))
    return allFeat
...
if __name__ == '__main__':
    ...
    features = compute_features(rawimage, read_in_images(initFrame, endFrame, filepath, fileFormatString), initFrame,
                                endFrame)
